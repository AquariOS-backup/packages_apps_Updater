...
updates_dir=/data/aquarios_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have aquarios-VERSION-DATE-TYPE-* as format"
...
# Assume aquarios-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.aquarios.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.aquarios.updater/databases/updates.db" \
